...
def Xenium_SVI(
    ...
    if kmeans_init:
        ...
        concentration_priors = torch.ones((len(gene_data), num_clusters))
    ...
    for i in range(num_elements):
        ...
        neighborhood_sum = concentration_priors[current_mask].sum(dim=0)
        # You can choose to normalize by the number of elements in the neighborhood if needed
        # neighborhood_count = current_mask.sum()
        ...
        spatial_concentration_priors[i] += neighborhood_sum
    ...
    def model(data):
        ...
        with pyro.plate("data", len(data), subsample_size=batch_size) as ind:
            ...
            cluster_means = pyro.sample("cluster_means", dist.LogNormal(0., 1.).expand([num_clusters, batch_data.size(1)]).to_event(2))
    ...
    def guide(data):
        ...
        with pyro.plate("data", len(data), subsample_size=batch_size) as ind:
            ...
            pyro.sample("cluster_means", dist.LogNormal(cluster_means_q, 0.1).to_event(2))
...
def parse_args():
    ...
    parser.add_argument("--spatial_normalize", type=str2bool, required=True)
    parser.add_argument("--concentration_amplification", type=int, required=True)
...
def main():
    ...
    # Call prepare_data with the appropriate arguments
    gene_data, spatial_locations, original_adata = prepare_data(
        dataset="hBreast", 
        spots=True, 
